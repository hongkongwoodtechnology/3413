...
export const POOL_ADDRESS = new PublicKey(
  process.env.NEXT_PUBLIC_POOL_WALLET?.trim() ||
    process.env.PLATFORM_PAYOUT_WALLET?.trim() ||
    "9FfHYyK8ZKsA82BPtierU4sWmwTS8QTGqrGqtTt6tEu7"
);
...
export function getDestinationAtaTargets(): { key: string; label: string; owner: PublicKey }[] {
  return [
    ...
    HOUSE_WALLET ? { key: 'house', label: '平台淨收益收款', owner: HOUSE_WALLET } : null,
    COMMISSION_WALLET ? { key: 'commission', label: '平台佣金收款', owner: COMMISSION_WALLET } : null,
  ].filter((target): target is { key: string; label: string; owner: PublicKey } => target !== null);
}
