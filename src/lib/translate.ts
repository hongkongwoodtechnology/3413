...
export async function translateTeamName(text: string, targetLang: SupportedLang): Promise<TranslateResult> {
  if (!text) return { translated: text, fromCache: true };

  const cacheKey = text.trim();
  if (!cacheKey) return { translated: text, fromCache: true };

  const cacheKeyWithLang = `${cacheKey}$${targetLang}`;
...
export async function translateToZh(text: string, targetLang: 'zh-TW' | 'zh-CN' = 'zh-TW'): Promise<TranslateResult> {
  return translateTeamName(text, targetLang);
}
...
const TRADITIONAL_TO_SIMPLIFIED: Record<string, string> = Object.fromEntries([
  ['後', '后'], ['麼', '么'], ['們', '们'], ['這', '这'], ['裡', '里'], ['為', '为'],
  ['說', '说'], ['沒', '没'], ['對', '对'], ['個', '个'], ['時', '时'], ['讓', '让'], ['過', '过'], ['開', '开'], ['來', '来'],
  ['種', '种'], ['還', '还'], ['樣', '样'], ['點', '点'],
  ['學', '学'], ['會', '会'], ['發', '发'],
  ['實', '实'], ['虛', '虚'], ['強', '强'], ['弱', '弱'], ['輕', '轻'], ['軟', '软'], ['硬', '硬'],
  ...
  ['滾', '滚'], ['臥', '卧'],
  ['進', '进'], ['歷', '历'], ['經', '经'], ['從', '从'],
  ['英格蘭', '英格兰'], ['曼徹斯特', '曼彻斯特'], ['阿森納', '阿森纳'], ['切爾西', '切尔西'],
  ...
  ['皇馬', '皇马'], ['巴塞', '巴萨'], ['巴塞隆納', '巴塞罗那'], ['馬德里', '马德里'],
  ['競技', '竞技'], ['華倫西亞', '瓦伦西亚'], ['維拉利爾', '比利亚雷亚尔'], ['貝迪斯', '贝蒂斯'],
  ['西維爾', '塞维利亚'], ['艾拉維斯', '阿拉维斯'], ['格塔菲', '赫塔费'], ['奧沙辛拿', '奥萨苏纳'],
  ['祖雲達斯', '尤文图斯'], ['AC米蘭', 'AC米兰'], ['國際米蘭', '国际米兰'],
  ...
  ['抽射', '抽射'], ['遠射', '远射'], ['頭球', '头球'], ['單刀', '单刀'],
  ['越位', '越位'], ['手球', '手球'], ['烏龍', '乌龙'], ['烏龍球', '乌龙球'], ['假摔', '假摔'],
  ['拖延', '拖延'], ['傷停', '伤停'], ['補時', '补时'], ['加時', '加时'],
  ['延長', '延长'], ['點球大戰', '点球大战'], ['互射', '互射'],
